...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  ,thisTable = function(){
    ...
    return {
      ...
      ,refresh: function (options) {
        that.refresh.call(that, options);
      },renderCustomData:function (data) {
        that.renderCustomData.call(that,data);
      }
  ...
  Class.prototype.resize = function(){
    ...
    if(that.config.onResize){
      that.config.onResize();
    }
  };
  ...
  /**
   *更新请求数据再次请求
   * where 存在则替换上次的请求参数，如果需要清空参数需要传{}
   * curr 请求的页码，不传则请求当前的页码
   */
  Class.prototype.refresh = function(options){
    var that = this;
    if(options){
      if(options.where){
        that.config.where = options.where;
      }
      if(options.curr){
        that.page = options.curr;
      }
    }
    that.pullData(that.page);
    if(that.config.page && that.config.page.curr){
      that.config.page.curr = that.page;
      laypage.render(that.config.page);
    }
  };

  Class.prototype.renderCustomData = function(data){
    var that = this;
  ...
  Class.prototype.fullSize = function(){
    ...
    //减去列头区域的高度
    bodyHeight = parseFloat(height) - ((that.layHeader && that.layHeader.outerHeight()) || 38); //此处的数字常量是为了防止容器处在隐藏区域无法获得高度的问题，暂时只对默认尺寸的表格做支持。
