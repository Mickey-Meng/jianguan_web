 ...
 */
layui.config({}).extend({}).define(['laytpl', 'laypage', 'layer', 'form','okUtils'], function(exports) {
	"use strict";
	var $ = layui.$,
		laytpl = layui.laytpl,
	...
	Class.prototype.tableHeight = function() {
		...
		if (!table.kit.isNumber(optionsFirst.height)) { //如果非固定高度，则计算高度
			...
			options.tempHeight = th - that.fullHeightGap;
		}
	};
	/**
	 * 重新渲染高度
	...
	Class.prototype.resizeHeight = function() {
		...
		var theader = options.isFilter ? 76 : 40; //没有行内过滤区域
