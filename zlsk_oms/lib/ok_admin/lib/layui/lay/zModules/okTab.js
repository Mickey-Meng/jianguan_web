...
layui.define(["element", "jquery"], function (exports) {
   ...
   function createMenu(data, tabID) {
      var menuTitle = data.NAME;
      var menuHref = sessionStorage.getItem("homeFolder") + data.URL;
      if (data.children != undefined && data.children.length > 0) {
         var menuIcon = data.ICON || '&#xe654;';
         temp += '<a>';
         if (menuIcon != undefined && menuIcon != '') {
            temp += ('<i class="z-icon">{0}</i>').format(menuIcon);
         }
         temp += '<cite>' + menuTitle + '</cite>';
         ...
         if(homePage && homePage.MENUCODE && data.MENUCODE  && data.MENUCODE=== homePage.MENUCODE){
            isClose = "false";
         }
         if (data.target) {
            temp += ('<a lay-id="{0}" data-url="{1}" target="{2}" is-close="{3}" data-mk="{4}">').format(tabID,menuHref, data.target, isClose,data.MENUCODE);
         } else {
            temp += ('<a lay-id="{0}" data-url="{1}" is-close="{2}" data-mk="{3}">').format(tabID,menuHref, isClose,data.MENUCODE);
         }
         if (menuIcon != undefined && menuIcon != '') {
            temp += ('<i class="z-icon">{0}</i>').format(menuIcon);
         }
         temp += ('<cite>{0}</cite></a>').format(menuTitle);
    ...
    * 通过配置新增页面
    */
   okTab.prototype.tabAddByConfig = function(config){
      var objE = document.createElement("div");
      var temp = '<a lay-id="'+ config.id +'"  is-close="'+ (config.isClose || 'true') +'" data-url="'+ config.url +'">';
      temp += ('<i class="z-icon">{0}</i>').format(config.icon);
      temp += '<cite>' + config.title + '</cite>';
      temp += '</a>';
      objE.innerHTML = temp;
      this.tabAdd(objE.childNodes);
   };
