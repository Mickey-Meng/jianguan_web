				...
				if(isUpLoading || isImporting){
					...
					if (!(window.ActiveXObject||"ActiveXObject" in window)){// 判断是否为IE
						...
						if(testFile){
							...
							alert("提示：您拖入的文件中包含了一个或多个地理目录，无法进行上传。");
						}
					}else{
	...
	if (check == "y") {
		...
		$.ajax({
			...
			success : function(result) {
				...
				var loginInfo = '{accountId:"' + 123 + '",accountPwd:"'
...
function showParentList(folderView) {
	...
	if(folderView.parentList.length>0){
		$.each(folderView.parentList, function(n, val) {
			var value = val.folderName === 'ROOT' ? '根目录' : val.folderName;
			$("#parentFolderList").append("<li><a href='javascript:void(0);' onclick='entryFolder("+'"' + val.folderId +'"'+")'>"+value+"</a></li>");
		});
	}else{
		$("#parentFolderList").html("<li class='disabled'><a>无</a></li>");
	}
	if(f.folderName.length>10){
		$("#currentFolderName").text(f.folderName.substr(0,6)+"...");
	}else{
		$("#currentFolderName").text(f.folderName === 'ROOT' ? '根目录' : f.folderName);
	}
...
var intro;
function initIntro() {
	layui.use(['okUtils','introJs'],function () {
		intro = layui.introJs();
		intro.setOptions({
			steps: [
				{
					element: "#introDropdown",
					intro: '<h5>地理目录切换</h5><br>' +
						'1.切换到当前数据或目录的父级目录<br>' +
						'2.如图所示,"<br><img src="./img/introSelecrParent.png">'
				},
				{
					element: "#filetableoptmenusreach",
					intro: '<h5>地理目录或数据搜索</h5><br>' +
						'1.在当前目录下搜索目录或数据文件<br>'
				},
				{
					element: "#introCreateFold",
					intro: '<h5>新建地理目录</h5><br>' +
						'1.根据行政区划创建相应名称地理目录<br>' +
						'2.如图所示,创建 "湖北省"<br><img src="./img/createFold.png">'
				},
				{
					element: "#introUploadFold",
					intro: '<h5>上传地理数据</h5><br>' +
						'1.上传3D Tiles文件目录到指定地理目录下<br>' +
						'2.如图所示,在 "湖北省/武汉市/洪山区/狮子山街道/" 地理目录下上传倾斜模型<br><img src="./img/uploadFold.png">'
				},
				{
					element: "#introUploadFold",
					intro: '<h5>复制链接</h5><br>' +
						'1.进入到上传的地理数据根目录<br>' +
						'2.找到 "tileset.json"文件,并点击 "复制URL" <br>' +
						'3.如图所示<br><img src="./img/copyUrl.png"><br>'
				},
				{
					element: "#introUploadFold",
					intro: '<h5>URL配置服务</h5><br>' +
						'1.进入左侧菜单  "地图配置" ,进行服务配置<br>' +
						'2.如图所示<br><img src="./img/copyUrlUse.png"><br>'
				}
			]
		});
		// intro.oncomplete(function () {
		// 	$('#introCreateFold').click();
		// });
		var hasShowIntro = layui.okUtils.getCookie('hasShowIntroFile');
		if(!hasShowIntro){
			intro.start();
			layui.okUtils.setCookie('hasShowIntroFile','hasShowIntroFile');
		}
	});
}

function showIntroduceAgain(){
	intro.start();
}
...
function showAccountView(folderView) {
	$("#tb,#tb2").html("");
	folderView.account = JSON.parse(sessionStorage.getItem("userInfo")).NAME;
...
function showFolderTable(folderView) {
	$("#foldertable").html("");
	if (parentpath != null && parentpath != "null") {
		// $("#foldertable").append("<tr onclick='returnPF()'><td><button onclick='returnPF()' class='btn btn-link btn-xs'>../</button></td><td class='hiddenColumn'>--</td><td>--</td><td class='hiddenColumn'>--</td><td>--</td></tr>");
		$("#foldertable").append("<tr onclick='returnPF()'><td><button onclick='returnPF()' class='btn btn-link btn-xs'>../</button></td><td class='hiddenColumn'>--</td><td>--</td><td>--</td></tr>");
	}
	var authList = folderView.authList;
	var aD = false;
	var aR = false;
	...
	for(var i2=folderView.fileList.length;i2>0;i2--){
		...
		if (aL) {
			...
			fileRow = fileRow
					+ "<button onclick='copyFilePath("
					+ '"'
					+ fi.filePath.replace(/\\/g,"/")
					+ '"'
					+ ")' class='btn btn-link btn-xs'><span class='glyphicon glyphicon-magnet'></span> 复制URL</button>";
...
function showDeleteFolderModel(folderId, folderName) {
	...
	$('#deleteFolderMessage').text(
			"提示：确定要彻底删除地理目录：[" + folderName + "]及其全部内容么？该操作不可恢复");
...
function renameFolder(folderId) {
	...
	var reg = new RegExp("[\/\|\\\\\*\\<\\>\\?\\:\\&\\$" + '"' + "]+", "g");
	if (newName.length == 0) {
		showRFolderAlert("提示：地理目录名称不能为空。");
	} else if (newName.length > 128) {
		showRFolderAlert("提示：地理目录名称太长。");
			...
			success : function(result) {
				if (result == "mustLogin") {
					...
					if (result == "noAuthorized") {
						...
						showRFolderAlert("提示：出现意外错误，可能未能编辑地理目录，请刷新后重试。");
					}
				}
			},
			error : function() {
		...
		showRFolderAlert("提示：地理目录名中不应含有：引号 / \ * | < > & $ : ? 且不能以“.”开头。");
	}
}
...
function checkUploadFile() {
	if(isUpLoading==false && isImporting == false){
		...
		showUploadFileAlert("提示：另一项上传数据或地理目录的任务尚未完成，无法开始上传");
	}
}
...
function dodownload(fileId) {
	...
	window.location.href = URL_PREFIX +  "homeController/downloadFile.do?fileId=" + fileId;
}
...
function getZFilePath() {
	var paths = window.location.pathname.split('/');
	var zFile = window.location.origin;
...
function pdfView(filePath) {
	window.open(getZFilePath() + "pdfview/web/viewer.html?file=/fileblocks/" + filePath);
}

// 预览Docx文档
function docxView(fileId){
	window.open(getZFilePath() + "pdfview/web/viewer.html?file=/resourceController/getWordView/" + fileId);
}

// 预览TXT文档
function txtView(fileId){
	window.open(getZFilePath() + "pdfview/web/viewer.html?file=/resourceController/getTxtView/" + fileId);
}
		...
		success : function(result) {
			if (result == "mustLogin") {
				...
				if (result == "noAuthorized") {
					...
					$('#moveFilesMessage').text("错误：不能将一个地理目录移动到其自身内部："+result.substring(20));
				} else {
					$('#moveFilesMessage').text("提示：出现意外错误，可能未能移动全部文件，请刷新后重试");
					$("#dmvfbutton").attr('disabled', false);
				}
...
function checkImportFolder(){
	if(isUpLoading == false && isImporting ==false){
		if(ifs != null && ifs.length > 0){// 必须选中文件
			...
			showImportFolderAlert("提示：您未选择任何地理目录，无法开始上传");
		}
	}else{
		showImportFolderAlert("提示：另一项上传文件或地理目录的任务尚未完成，无法开始上传");
	}
}
	...
	$.ajax({
		...
		success:function(result){
			if(result == 'deleteSuccess'){
				...
				showImportFolderAlert("提示：无法覆盖原地理目录，上传失败");
			}
		},
		error:function(){
			showImportFolderAlert("提示：无法覆盖原地理目录，上传失败");
	...
	$.ajax({
		...
		success:function(result){
			...
			if(resJson.result == 'success'){
				...
				showImportFolderAlert("提示：生成新地理目录名称失败，无法开始上传");
			}
		},
		error:function(){
			showImportFolderAlert("提示：生成新地理目录名称失败，无法开始上传");
...
function copyFilePath(filePath) {
	filePath = window.location.origin + '/data_cesium/' + filePath;
	var oInput = document.createElement('input');
	oInput.value = filePath;
