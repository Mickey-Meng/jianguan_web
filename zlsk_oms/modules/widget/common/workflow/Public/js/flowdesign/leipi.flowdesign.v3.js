  ...
  var setConnections = function(conn, remove) {
   ...
   $.fn.Flowdesign = function(options)
    ...
    if(processData.list)
    {
        $.each(processData.list, function(i,row)
        {
            ...
            .mousedown(function(e){
              if( e.which == 3 ) { //右键绑定
                  _canvas.find('#leipi_active_id').val(row.pid);
    ...
    $(".process-step").live('click',function(){
        //激活
        _canvas.find('#leipi_active_id').val($(this).attr("pid")),
    ...
    var Flowdesign = {
        ...
        addProcess:function(row){
            // console.log(row.nodeid);
                ...
                var nodeDiv = document.createElement('div');
                var nodeId = "window" + row.nodeid, badge = 'badge-inverse',icon = 'icon-star';
                var style = "left:"+row.x+"px;top:"+row.y+"px;color:#0e76a8;"
                // if(row.icon)
                // {
                //     icon = row.icon;
                // }
                ...
                .attr("style",style)
                .attr("process_to","") //row.process_to
                .attr("process_id",row.nodeid)
                .addClass("process-step btn btn-small")
                .html('<span class="process-flag badge '+badge+'"><i class="'+icon+' icon-white"></i></span>&nbsp;' + row.name )
                .mousedown(function(e){
                  if( e.which == 3 ) { //右键绑定
            ...
            try{
              ...
              $("#leipi_process_info input[type=hidden]").each(function(i){
                  ...
                  if(processVal.length==2)
                  {
                    if(!aProcessData[processVal[0]])
                    {
                        ...
                        aProcessData[processVal[0]] = {"y":0,"x":0,"enode":[]};
                    }
                    // aProcessData[processVal[0]]["process_to"].push(processVal[1]);
                    aProcessData[processVal[0]]["enode"].push(processVal[1]);
                  }
              })
              /*位置*/
              _canvas.find("div.process-step").each(function(i){ //生成Json字符串，发送到服务器解析
                      if($(this).attr('id')){
                          ...
                          var totalId = $(this).attr('pid')
                         if(!aProcessData[pId])
                          {
                            // aProcessData[pId] = {"top":0,"left":0,"process_to":[]};
                            aProcessData[pId] = {"id":0,"y":0,"x":0,"snode":0,"enode":[]};
                          }
                        //   aProcessData[pId]["top"] =pTop;
                        //   aProcessData[pId]["left"] =pLeft;
                          aProcessData[pId]["id"] =totalId;
                          aProcessData[pId]["y"] =pTop;
                          aProcessData[pId]["x"] =pLeft;
                          aProcessData[pId]["snode"] =pId;
                      }
                  })
