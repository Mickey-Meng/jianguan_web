...
var showMeasureResult = function(result, type) {
    ...
    if (transparencyBalloons && transparencyBalloons != null) {
    ...
    if ($('div[tag="ViewTranSetting"]').hasClass('selected') || $('div[tag="EffectRain"]').hasClass('selected')
        || $('div[tag="EffectSnow"]').hasClass('selected') || $('div[tag="EffectFog"]').hasClass('selected')) {
        htmlBal.SetScreenLocation(500, 0);
    }else{
    ...
    if (ba < 0) {
        ...
        htmlBal.SetScreenLocation(0, -20);
...
var bTerrainClick = function() {
     ...
     * 返回：无
     */
var horizontalDisClick = earthCtx.horizontalDisClick = function() {
    hideBollon();
    earth.Event.OnMeasureFinish = showMeasureResult;//回调函数：当测量玩成的时候调用showMeasureResult
    setTimeout(function() {
        earth.Measure.MeasureHorizontalDistance(bTerrain);//水平距离测量接口，忽略地形
    }, 100);
 ...
 * 返回：无
 */
var verticalDisClick = earthCtx.verticalDisClick = function() {
    hideBollon();
 ...
 * 返回：无
 */
var spaceDisClick = earthCtx.spaceDisClick = function() {
    hideBollon();
    earth.Event.OnMeasureFinish = showMeasureResult;
 ...
 * 管线测量对象
 */
  var getLayerKey = function(layerName,t) {
        var lkl = SFM.queryHelper.getLayerKey(layerName,t);
        if (lkl && lkl.length > 0) {
            return lkl;
        }
        return null;
...
var PipelineMeasure = {
    ...
    getPipeLocalInfo: function(pipelineId, key) {
        ...
        params.ReturnDataType = localSearchDataType.xml;
     ...
     */
     getPipelineCoord: function(result,layerGuid,key) {
        ...
        var recordNode = result.attributes;
        var us_SDEEP = recordNode.ST_SALT;
        var us_EDEEP = recordNode.ST_EALT;
        var us_SDEEP_value = parseFloat(us_SDEEP); //起点高程
        var us_EDEEP_value = parseFloat(us_EDEEP); //止点高程
        //排水时管底，非排水是管顶
        var us_Size = recordNode.ST_SIZE;
        var us_Size_value = us_Size;
        var pipeHeight = us_Size_value;
        var pipeWidth = us_Size_value;
        var usSizeStr = us_Size_value + '';
        if(usSizeStr.indexOf('X') > -1){
        ...
        var jsonStr = result.geometry.geometryJSON ;
        ...
        for (var i = 0; i < coors.length; i++) {
            ...
            //var coordPoint = this.datum.des_BLH_to_src_xy(coors[i][0],coors[i][1]);
            var datum = SYSTEMPARAMS.pipeDatum;
            var point = {x:coors[i][0],y:coors[i][1]};
            var coordPoint =  SFM.queryHelper.xy2blh(datum,point);
    ...
    getPipeLineMeshVertices: function(layer, key){
        ...
        for (var i = 0, len = layer.GetChildCount(); i < len; i++) {
            ...
            if (curlayer.LayerType === "Container" || subLayer.LayerType === "Model_container") { // 使用具体的_container图层
                break;
            }
        }
        if (curlayer == null) {
        ...
        curlayer.LocalSearchParameter.ReturnDataType = localSearchDataType.xml;
    ...
    onPickObjectEvent: function(callback) {
        ...
        earth.Event.OnPickObjectEx = function(obj) {
            ...
            //var useKey = getName("US_KEY", 1, true);
            var useKey = obj.GetKey();
            ...
            /*var name = layer.Name;
            var lk = getLayerKey(name,'line');*/
            var lk = getLayerKey(layer.Name,'line');
            SFM.queryHelper.queryByKey(lk.join(','),useKey).then(function(data){
                   var d = data.getMe;
                   var res = d[0];
                   var coordObj = PipelineMeasure.getPipelineCoord(res,newLayerID,useKey);
 ...
 * 返回：无
 */
var pipelineHorDisClick = earthCtx.pipelineHorDisClick = function() {
    hideBollon();
    PipelineMeasure.clearPipelineMeasure();
    PipelineMeasure.onPickObjectEvent(function() {
        ...
        //alert(coordinate1.centerLineVect3s.Count + " " + coordinate2.centerLineVect3s.Count);
        // var p1 = coordinate1.coordList[0];
        // var p2 = coordinate1.coordList[1];
        // var q1 = coordinate2.coordList[0];
        // var q2 = coordinate2.coordList[1];
        // if((PipelineMeasure.isCoordPointEqual(p1, q1) == true) ||
        //    (PipelineMeasure.isCoordPointEqual(p1, q2) == true) ||
        //    (PipelineMeasure.isCoordPointEqual(p2, q1) == true) ||
        //    (PipelineMeasure.isCoordPointEqual(p2, q2) == true)){
        // 	alert("管线端点重合,两个管线相接");
        // 	return;
        // }else{
        // 	var isIntersect = PipelineMeasureAlgorithm.IsSegmentIntersect(p1, p2, q1, q2);
        // 	if(isIntersect == true){ //判断两条线段是否相交
        //               showMeasureResult(0,300);
        //               return;
        // 	}else{
        // 		var pipeHorDis = PipelineMeasureAlgorithm.SegmentMinDistance(p1, p2, q1, q2);
        // 		pipeHorDis = pipeHorDis;
        //               showMeasureResult(pipeHorDis,301);
        //               return;
        // 	}
        // }
    });
 ...
 * 返回：无
 */
var pipelineVerDisClick = earthCtx.pipelineVerDisClick = function() {
    hideBollon();
    PipelineMeasure.clearPipelineMeasure();
    PipelineMeasure.onPickObjectEvent(function() {
        var coordinate1 = PipelineMeasure.coordinate1;
        ...
        setTimeout(function(){
            var horDis = earth.GeometryAlgorithm.CalculateLineLineDistance(coordinate1.centerLineVect3s, coordinate2.centerLineVect3s);
            // var spaceDis = earth.GeometryAlgorithm.CalculateLineLineSpaceDistance(coordinate1.centerLineVect3s, coordinate2.centerLineVect3s);
            var spaceDis = earth.GeometryAlgorithm.CalculateMeshDistance(coordinate1.lineMeshV3s, coordinate2.lineMeshV3s);
            if (horDis && spaceDis > 0) {
                var horDisLengthNum = horDis.Length - coordinate1.pipeWidth*0.5 - coordinate2.pipeWidth*0.5;
                var tempValue = Math.pow(spaceDis, 2) - Math.pow(horDisLengthNum, 2);
                var verDis = 0;
                if (tempValue >= 0) {
                    verDis = Math.sqrt(tempValue);
                    verDis = verDis - coordinate1.pipeHeight*0.5 - coordinate2.pipeHeight*0.5;
                    if(verDis < 0){
                        verDis = 0;
                    }
                }
                ...
                // lineObj.BeginUpdate();
                // lineObj.SetPointArray(spaceDis);
        ...
        /* if(spaceDis!=null){
             showMeasureResult(spaceDis.Length,302);
         }else{
             showMeasureResult(0,302);
         }*/
        ...
        // 	var isIntersect = PipelineMeasureAlgorithm.IsSegmentIntersect(p1, p2, q1, q2);
        // 	if(isIntersect == true){ //判断两条线段是否相交
        // 		var p = PipelineMeasureAlgorithm.segmentIntersect(p1, p2, q1, q2);
        // 		// 计算垂直净距
        //               var src_len1 = PipelineMeasureAlgorithm.Vector2Sub(p, p1).Length; //(pointIntersect - pipeList[0]).Length();
        //               var src_len2 = PipelineMeasureAlgorithm.Vector2Sub(p, p2).Length; //(pointIntersect - pipeList[1]).Length();
        //               var src_alt = p1.Z + (p2.Z - p1.Z) * src_len1 / (src_len1 + src_len2);

        //               var des_len1 = PipelineMeasureAlgorithm.Vector2Sub(p, q1).Length; //(pointIntersect - pipeList[2]).Length();
        //               var des_len2 = PipelineMeasureAlgorithm.Vector2Sub(p, q2).Length; //(pointIntersect - pipeList[3]).Length();
        //               var des_alt = q1.Z + (q2.Z - q1.Z) * des_len1 / (des_len1 + des_len2);
        //               var pipeVerDis = Math.abs(src_alt - des_alt);
        //               showMeasureResult(pipeVerDis,302);
        //               return;
        // 	}else{
        // 		//不相交的计算法有点不同
        //               var alt1 = p1.Z - q1.Z;
        ...
        //               var alt4 = p2.Z - q2.Z;
        //               var pipeVerDis = null;
        //               if (alt1 * alt2 <= 0 || alt3 * alt4 <= 0 || alt1 * alt3 <= 0){
        //                   pipeVerDis = 0;
        //               }else{
        //                   pipeVerDis = Math.min(Math.abs(alt1), Math.abs(alt2));
        //                   pipeVerDis = Math.min(Math.abs(alt3), pipeVerDis);
        //                   pipeVerDis = Math.min(Math.abs(alt4), pipeVerDis);
        //                   if (pipeVerDis < 0){
        //                       pipeVerDis = 0;
        //                   }
        //               }
        //               pipeVerDis = pipeVerDis;
        //               //var res ="管线不相交,管间垂直距离为: " + pipeVerDis + " 米";
        //               showMeasureResult(pipeVerDis,303);
        //               return;
        // 	}
        // }
    });
 ...
 * 返回：无
 */
var pipelineSpaceDisClick =earthCtx.pipelineSpaceDisClick = function() {
    hideBollon();
    PipelineMeasure.clearPipelineMeasure();
    PipelineMeasure.onPickObjectEvent(function() {
        var coordinate1 = PipelineMeasure.coordinate1;
        ...
        // 	return;
        // }else{
        // 	var pipeSpaceDis = PipelineMeasureAlgorithm.SegmentMinDistance(p1, p2, q1, q2);
        // 	pipeSpaceDis = pipeSpaceDis;
        //           var res ="管间空间距离为: " + pipeSpaceDis + " 米";
        //           showMeasureResult(pipeSpaceDis,304);
        // }
    });
};
//----------------------------------------------------------------------
 ...
 * 返回：无
 */
var flatAreaClick = earthCtx.flatAreaClick = function() {
    hideBollon();
    earth.Event.OnMeasureFinish = showMeasureResult;
    setTimeout(function() {
 ...
 * 返回：无
 */
var mPlaneAngleClick = earthCtx.mPlaneAngleClick = function() {
    hideBollon();
