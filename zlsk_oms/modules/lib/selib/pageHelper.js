...
Query.PageHelper = function(curEarth){
	...
	var getFieldValue = function(header, aliasHeader, pageRecord){
		//alert(ectx.getName("US_KEY", 1, true));
		var values= [];
		columnName = [];
		aliasColumnName = [];
				...
				if (record) {
					...
					for (var j = 0; j < header.length; j++) {
						...
						if(key != "US_FEATURE" && key != "layerName"){
							if(resType === "管点"){
								key = ectx.getName(key, 0, true);
							}else{
								key = ectx.getName(key, 1, true);
							}
						}
	                    ...
	                    if(key && (record[key] != undefined || record[keyUpper] != undefined)){
	                    	...
	                    	if(key === "US_FEATURE"){
	                    		...
	                    		if(resType === "管点"){
									var usType = ectx.getName("US_PT_TYPE", 0, true);
									var usAttach = ectx.getName("US_ATTACHMENT", 0, true);
									var usWell = ectx.getName("US_WELL", 0, true);
	...
	var initDataGrid = function(header, aliasHeader, pageRecord,importExcelBtn){
		...
		$("#dg").datagrid({
            ...
            onDblClickRow: function(rowIndex, rowData){
				...
				var guid;
				if(type === "管线"){
					key = ectx.getName("US_KEY", 1, true);
					guid = ectx.getName("US_ID", 1, true);
				}else{
					key = ectx.getName("US_KEY", 0, true);
					guid = ectx.getName("US_ID", 0, true);
				}
				...
				if(bShow){
                    ...
                    if(type != "管线"){
                        ...
                        var strKey=record[ectx.getName("US_KEY",0,true)];
                        var road=record[ectx.getName("US_ROAD",0,true)];
                        var isScra=record[ectx.getName("US_IS_SCRA",0,true)];
                        var bdTime=record[ectx.getName("US_BD_TIME",0,true)];
                        var fxYear=record[ectx.getName("US_FX_YEAR",0,true)];
                        var owner=record[ectx.getName("US_OWNER",0,true)];
                        var state=record[ectx.getName("US_UPDATE",0,true)];
                        var update=record[ectx.getName("US_UPDATE",0,true)];
                        var altitude=(parseFloat(record[ectx.getName("US_PT_ALT",0,true)])).toFixed(3);
                        var attachment = record[ectx.getName("US_ATTACHMENT",0,true)];
                        var pointType = record[ectx.getName("US_PT_TYPE",0,true)];

                        var str_caption=ectx.getNameNoIgnoreCase("US_KEY",0,false);
                        var road_caption=ectx.getNameNoIgnoreCase("US_ROAD",0,false);
                        var isScra_caption=ectx.getNameNoIgnoreCase("US_IS_SCRA",0,false);
                        var bdTime_caption=ectx.getNameNoIgnoreCase("US_BD_TIME",0,false);
                        var fxYear_caption=ectx.getNameNoIgnoreCase("US_FX_YEAR",0,false);
                        var owner_caption=ectx.getNameNoIgnoreCase("US_OWNER",0,false);
                        var state_caption=ectx.getNameNoIgnoreCase("US_UPDATE",0,false);
                        var update_caption=ectx.getNameNoIgnoreCase("US_UPDATE",0,false);
                        var altitude_caption=ectx.getNameNoIgnoreCase("US_PT_ALT",0,false);
                        var attachment_caption = ectx.getNameNoIgnoreCase("US_ATTACHMENT",0,false);
                        var pointType_caption = ectx.getNameNoIgnoreCase("US_PT_TYPE",0,false);

                        //井类型 井直径 井脖深 井底深 井盖类型  井盖规格 井盖材质  井材质  旋转角度  偏心井点号
                        var us_well=record[ectx.getName("US_WELL",0,true)];
                        var us_wdia=record[ectx.getName("US_WDIA",0,true)];
                        var us_ndeep=(parseFloat(record[ectx.getName("US_NDEEP",0,true)])).toFixed(3);
                        var us_wdeep=(parseFloat(record[ectx.getName("US_WDEEP",0,true)])).toFixed(3);
                        var us_plate=record[ectx.getName("US_PLATE",0,true)];
                        var us_psize=(parseFloat(record[ectx.getName("US_PSIZE",0,true)])).toFixed(3);
                        var us_pmater=record[ectx.getName("US_PMATER",0,true)];
                        var us_wmater=record[ectx.getName("US_WMATER",0,true)];
                        var us_angle=record[ectx.getName("US_ANGLE",0,true)];
                        var us_offset=record[ectx.getName("US_OFFSET",0,true)];

                        var us_well_caption=ectx.getNameNoIgnoreCase("US_WELL",0,false);
                        var us_wdia_caption=ectx.getNameNoIgnoreCase("US_WDIA",0,false);
                        var us_ndeep_caption=ectx.getNameNoIgnoreCase("US_NDEEP",0,false);
                        var us_wdeep_caption=ectx.getNameNoIgnoreCase("US_WDEEP",0,false);
                        var us_plate_caption=ectx.getNameNoIgnoreCase("US_PLATE",0,false);
                        var us_psize_caption=ectx.getNameNoIgnoreCase("US_PSIZE",0,false);
                        var us_pmater_caption=ectx.getNameNoIgnoreCase("US_PMATER",0,false);
                        var us_wmater_caption=ectx.getNameNoIgnoreCase("US_WMATER",0,false);
                        var us_angle_caption=ectx.getNameNoIgnoreCase("US_ANGLE",0,false);
                        var us_offset_caption=ectx.getNameNoIgnoreCase("US_OFFSET",0,false);
                        ...
                        var v3s=null;
                        var us_key = ectx.getName("US_KEY",0,true);
                        ...
                        earth.Event.OnEditDatabaseFinished = function(pRes, pFeature){
                            if (pRes.ExcuteType == parent.excuteType){
                                ...
                                var str = "";
                                str += '<tr><td style="word-wrap:break-word;" width="100">&nbsp;&nbsp;&nbsp;&nbsp;'+str_caption+'</td><td style="word-wrap:break-word;" width="150">&nbsp;&nbsp;&nbsp;&nbsp;'+ "   " +record[ectx.getName("US_KEY",0,true)]+'</td></tr>';
        ...
        if(values && values.length){
            var standardLineKey = ectx.getName("US_KEY", 1, true);
            var standardPointKey = ectx.getName("US_KEY", 0, true);
            for(var i = 0; i < values.length; i++){
                var item = values[i];
                for(var key in item){
    ...
    function  transformToPlaneCoordinates(layerId,coord){
        var datum=  ectx.SYSTEMPARAMS.pipeDatum;
    ...
    var initLineValue = function (layerID, record, layerName){
        var US_PMATER=(ectx.getName("US_PMATER",1,true));
        var us_pmater_caption=(ectx.getName("US_PMATER",1,false));
        var material = record[US_PMATER];//管线材质

        var US_LTTYPE=(ectx.getName("US_LTTYPE",1,true));
        var US_LTTYPE_caption=(ectx.getName("US_LTTYPE",1,false));
        var lineType=record[US_LTTYPE];//埋设类型

        var US_PDIAM=(ectx.getName("US_SIZE",1,true));
        var US_PDIAM_caption=(ectx.getName("US_SIZE",1,false));
        var diam=record[US_PDIAM];

        var US_IS_SCRA=(ectx.getName("US_IS_SCRA",1,true));
        var US_IS_SCRA_caption=(ectx.getName("US_IS_SCRA",1,false));
        var isScra=record[US_IS_SCRA];
        var US_BD_TIME=(ectx.getName("US_BD_TIME",1,true));
        var US_BD_TIME_caption=(ectx.getName("US_BD_TIME",1,false));
        var bdTime=record[US_BD_TIME];

        var US_FX_YEAR=(ectx.getName("US_FX_YEAR",1,true));
        var US_FX_YEAR_caption=(ectx.getName("US_FX_YEAR",1,false));

        var US_STATE=(ectx.getName("US_STATUS",1,true));
        var US_STATE_caption=(ectx.getName("US_STATUS",1,false));
        var state=record[US_STATE];

        var US_UPDATE=(ectx.getName("US_UPDATE",1,true));
        var US_UPDATE_caption=(ectx.getName("US_UPDATE",1,false));
        var update= record[US_UPDATE];

        var US_OWNER=(ectx.getName("US_OWNER",1,true));
        var US_OWNER_caption=(ectx.getName("US_OWNER",1,false));
        var owner=record[US_OWNER];

        var US_ROAD=(ectx.getName("US_ROAD",1,true));
        var US_ROAD_caption=(ectx.getName("US_ROAD",1,false));
        var road=record[US_ROAD];
        var str_caption=ectx.getNameNoIgnoreCase("US_KEY",1,false);
        if(bdTime==undefined){
            bdTime="";
        }
        if(state==undefined){
        ...
        var str = "";
        str += '<tr><td  width="100">&nbsp;&nbsp;&nbsp;&nbsp;' + str_caption +'</td><td>&nbsp;&nbsp;&nbsp;&nbsp;'+record[ectx.getName("US_KEY",1,true)]||""+'</td></tr>';
        ...
        if ((intLayerCode >= 5000 && intLayerCode < 6000)||(intLayerCode >= 6000 && intLayerCode < 7000)||(intLayerCode >= 7000 && intLayerCode < 8000)){
            var pressur= record[ectx.getName("US_PRESSUR",1,true)];//压力
            var pressur_caption= ectx.getName("US_PRESSUR",1,false);
            str += '<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;'+pressur_caption+'</td><td>&nbsp;&nbsp;&nbsp;&nbsp;'+(pressur!=undefined?pressur:"")+'</td></tr>';
        }
        //压力
        if(intLayerCode >= 1000 && intLayerCode < 2000){
            var voltage= record[ectx.getName("US_PRESSUR",1,true)];
            var voltage_caption= ectx.getName("US_PRESSUR",1,false);
        ...
        if ((intLayerCode >= 4000 && intLayerCode < 5000)||(intLayerCode >= 7000 && intLayerCode < 8000)){
            var flower=record[ectx.getName("US_FLOWDIR",1,true)];
            var flower_caption=ectx.getName("US_FLOWDIR",1,false);
        ...
        if ((intLayerCode >= 1000 && intLayerCode < 2000)||(intLayerCode >= 2000 && intLayerCode < 3000)){
            var ventnum=record[ectx.getName("US_VENTNUM",1,true)];
            var holeto=record[ectx.getName("US_HOLETOL",1,true)];
            var holeused=record[ectx.getName("US_HOLEUSE",1,true)];
            var ventnum_caption=ectx.getName("US_VENTNUM",1,false);
            var holeto_caption=ectx.getName("US_HOLETOL",1,false);
            var holeused_caption=ectx.getName("US_HOLEUSE",1,false);
            //电压值
            var US_PSVALUE=(ectx.getName("US_PSVALUE",1,true));
            var US_PSVALUE_caption=(ectx.getName("US_PSVALUE",1,false));
    ...
    var stopHighLight = function(){
    	for(var k = 0;ectx.earth!=null&&k < highLightObjs.length; k++){
	...
	var highlightObject = function (layerID, type, guid, key) {
        ...
        for (i = 0; i < layer.GetChildCount(); i++) {
            ...
            if (obj != null) {
                ...
                earth.GlobeObserver.GotoLookat(vecCenter.X, vecCenter.Y, vecCenter.Z, 50.0, 45.3, 0, 20);
                obj.ShowHighLight();
                highLightObjs.push(obj);
                //显示气泡
                if(bShow){
                    parent.showHtmlBalloon(vecCenter.X, vecCenter.Y, vecCenter.Z, htmlStr);
                }
                return;
            }
        }
        ...
        if (obj == null && type === "point") {
           // StatisticsMgr.sphereGotoLookat(guid, subLayer, layerID, key);
            StatisticsMgr.sphereGotoLookat(guid, layer.GetChildAt(4), layerID, key, bShow, originCoord, htmlStr);
        }
        //if (obj == null && type === "管线")
        //{
