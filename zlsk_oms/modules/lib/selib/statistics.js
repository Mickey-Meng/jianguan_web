...
var bFlagIndex = 0;
var id_timeout;
var StatisticsMgr = earthCtx.StatisticsMgr = {
    ...
    initPipelineList1: function(projectId, container, callback) {
        container.html("");
        var layer = earth.LayerManager.GetLayerByGUID(projectId);
        var pipelineList = this.getPipeListByLayer(layer);
        /*var pipelineList = null;
        ...
        for (var i = 0; i < pipelineList.length; i++) {
            var pipeLineLayer = pipelineList[i];
            container.append('<div style="width:180px;overflow: auto;">' +
                '<input type="checkbox" style="margin-top:5px;" server="' + pipeLineLayer.server + '" id="' + pipeLineLayer.id + '" value="' + pipeLineLayer.id + '" name="' + pipeLineLayer.name + '" />' +
                '<label style="margin-left:15px;" for="' + pipeLineLayer.id + '">' + pipeLineLayer.name + '</label>' +
                '</div>');
        }
     ...
     */
    showClassificationResult2: function(classResList, container, columnNum) {
        ...
        for (var i = 0; i < classResList.length; i++) {
            ...
            if (classLayer.dataList.length > 1) {
                for (var k = 0; k < classLayer.dataList.length; k++) {
                    ...
                    for (var attr in dataObj) {
                        if (attr != "layerName") {
                            htmlStr = htmlStr + '<td align="center" style="' + tdCss + '" width="' + 100 / columnNum + '%">' + dataObj[attr] + '</td>';
                        } else {
                            htmlStr = htmlStr + '<td rowspan="' + classLayer.dataList.length + '" style="' + tdCss + '" align="center" width="' + 100 / (columnNum + 1) + '%">' + dataObj[attr] + '</td>';
                        }
                    }
                    htmlStr = htmlStr + '</tr>';
                }
    ...
    showRangeList: function(container, rangeList) {
        ...
        for (var k = 0; k < rangeList.length; k++) {
            htmlStr = htmlStr + '<tr><td style="text-align:left; cursor:default;" onclick="ectx.StatisticsMgr.selectSingleRow(this)">' + rangeList[k] + '</td></tr>';
        }
        htmlStr = htmlStr + '</table>';
        container.html(htmlStr);
    },
     showRangeList1: function(container, rangeList) {
        container.html("");
        var htmlStr = '<table style="width:100%;" cellspacing="1">';
        container.append();
        for (var k = 0; k < rangeList.length; k++) {
            htmlStr = htmlStr + '<tr><td style="text-align:left; cursor:default;" onclick="ectx.StatisticsMgr.selectSingleRow1(this)">' + rangeList[k] + '</td></tr>';
        }
        htmlStr = htmlStr + '</table>';
        container.html(htmlStr);
    },
    ...
    selectSingleRow: function(obj) {
     ...
     selectSingleRow1: function(obj) {
        var trObj = obj.parentNode;
        var tableObj = trObj.parentNode;
        for (var i = 0; i < tableObj.rows.length; i++) {
            tableObj.rows[i].style.color = "white";
            tableObj.rows[i].style.backgroundColor = "transparent";
        }
        // trObj.style.color = "#ffffff";
        // trObj.style.backgroundColor = "#316ac5";
         trObj.style.color = "white";
         trObj.style.backgroundColor = "rgb(0,120,215)";
        tableObj.parentNode.selectIndex = trObj.rowIndex;
    },
    /**
     * 功能：使表单元格处于可编辑状态
