...
STAMP.Userdata = function(earthObj) {
    ...
    /**
     *  每次添加对象的时候不再执行initTree 执行数据更新即可
     */
    function updateTree(data) {
        ...
        for (var i = 0; i < nodes.length; ++i) {
            ...
            if (data.type == 292) {
                ...
                } else if (name == "填挖方模型数据") {
                    if (data.excavation) {
                        root = node;
                        break;
                    }
                } else if (name == "深度利用分析模型数据") {
                    if (data.depthLayout) {
                        root = node;
                        break;
                    }
                } else if (name == "地质模型数据") {
                    if (!data.separated && !data.withTunnel &&
                        !data.cut && !data.excavation &&
                        !data.depthLayout) {
                        root = node;
                        break;
                    }
     ...
     */
    var _createMilitaryTag = function(flag, userdataTree, u) {
        var obj = {};
        obj.action = "add";
        obj.earth = earth;
        obj.name = flag;
        earth.Event.OnCreateGeometry = function(pval, type) {
            ...
            if (obj.type === "250" || obj.type === "251" || obj.type === "252" || obj.type === "253" || obj.type === "254" || obj.type === "255" || obj.type === "256" || obj.type === "260") {
                militaryData = window.showModalDialog(u, obj, "dialogWidth=400px;dialogHeight=450px;status=no");
            } else {
                militaryData = window.showModalDialog(u, obj, "dialogWidth=400px;dialogHeight=350px;status=no");
            }
    ...
    var _importModelData = function(flag, userdataTree) {
        ...
        if (flag === "model" || flag === "tree" || flag === "match") {
            earth.Event.OnCreateGeometry = function(pVal) {
                ...
                obj.tag = tag;
                var rValue = showModalDialog("html/userdata/modelData.html", obj, "dialogWidth=370px;dialogHeight=240px;status=no");
                if (obj.click === "false") {
                    earth.ShapeCreator.Clear();
    ...
    //  */
    // var createElement = function (obj, element, editMsg) {
    //     var dataAry = getuserdataArr();
    //     if (dataAry) {
    //         if (dataAry.length) {
    //             obj.name = obj.name + '_' + dataAry.length;
    //         }
    //     }
    //     var rootxml = getUserdata(filename);
    //     var elementXML = getElementByGUID(rootxml, obj.guid);
    ...
    //         var docNode;
    //         if (rootxml.childNodes.length > 1) {
    //             docNode = rootxml.childNodes[rootxml.childNodes.length - 1];
    //         } else {
    //             docNode = rootxml.documentElement;
    //         }
    //         for(var i = 0; i < docNode.childNodes.length; ++i) {
    //             var child = docNode.childNodes[i];
    ...
    //                     }
    //                 } else if (name == "地质模型数据") {
    //                     if (!obj.separated && !obj.withTunnel &&
    //                             !obj.cut && !obj.excavation &&
    //                             !obj.depthLayout) {
    //                         lookupNode = child;
    //                         break;
    //                     }
    ...
    var createElementXml = function(obj, elment) {
        ...
        if (obj.type === 229) { //model
        ...
        } else if (obj.type == 292) {
            heading = 0;
            tilt = 90;
            range = 1000;

        } else if (obj.type == 291) {
            xmlData += " <Separated>" + obj.separated + "</Separated>";
            xmlData += " <TunnelAttached>" + obj.tunnelAttached + "</TunnelAttached>";
            if (obj.tunnelAttached) {
                xmlData += "  <TunnelWidth>" + obj.tunnelWidth + "</TunnelWidth>";
                xmlData += "  <TunnelHeight>" + obj.tunnelHeight + "</TunnelHeight>";
                xmlData += "  <TunnelDepth>" + obj.tunnelDepth + "</TunnelDepth>";
                xmlData += "  <TunnelPoints>";
                xmlData += "   <Count>" + obj.tunnelPoints.Count + "</Count>";
                for (var i = 0; i < obj.tunnelPoints.Count; ++i) {
                    var point = obj.tunnelPoints.Items(i);
                    xmlData += "   <Point>";
                    xmlData += "<X>" + point.X + "</X>";
                    xmlData += "<Y>" + point.Y + "</Y>";
                    xmlData += "<Z>" + point.Z + "</Z>";
            ...
            xmlData += " <LayerCount>" + obj.layers.length + "</LayerCount>";

            for (var i = 0; i < obj.layers.length; ++i) {
                xmlData += " <Layer>";
                xmlData += "  <Name>" + obj.layers[i].Name + "</Name>";
                xmlData += "  <Color>" + obj.layers[i].Color + "</Color>";
                xmlData += "  <PolyhedronCount>" + obj.layers[i].PolyhedronCount + "</PolyhedronCount>"
                xmlData += " </Layer>";
            }
            ...
            xmlData += "  <TextHorizontalScale>" + obj.textHorizontalScale + "</eTxtHorizontalScale>";
        ...
        } else if (obj.type == 210) {} else if (obj.type === 280) {
        ...
        if (undefined == heading) {
            heading = earth.GlobeObserver.Pose.heading;
        }
        if (undefined == tilt) {
            tilt = earth.GlobeObserver.Pose.tilt;
        }
        if (undefined == range) {
            range = earth.GlobeObserver.PickRange();
        }
    ...
    var _editUserNode = function(id, filename) {
        ...
        var u = SFM.sfmUtil.getMainWin().SFM.sfmUtil.getHostUrl() + "/modules/widget/common/plot/Arrow/";
        ...
        for (var i = 0; i < userdataArr.length; i++) { //这里的length不对???
            ...
            if (eleid === id) {
                ...
                if (particleType === "fire" || particleType === "mist" || particleType === "fountain" || particleType === "nozzle" || particleType === "SprayNozzle" || particleType === "dWater") {
                    ...
                    window.showModalDialog(u+"getParticleName.html", obj, "dialogWidth=400px;dialogHeight=110px;status=no");
                ...
                if (obj.type === 220 || obj.type === 239 || obj.type === 243 || obj.type === 240 || obj.type === 241 || obj.type === 211 || obj.type === 245) {
                    ...
                    if (obj.type === 245) {
                        ...
                        obj.linecolor = "#" + currentElement.LineStyle.LineColor.toString(16);
                        obj.fillcolor = "#" + currentElement.FillStyle.FillColor.toString(16);
                        window.showModalDialog(u+"texturePolygon.html", obj, "dialogWidth=400px;dialogHeight=540px;status=no");
                        if (obj.click === "false") {
                            return;
                        }
                        ...
                        if (obj.type === 220 || obj.type === 239 || obj.type === 241 || obj.type == 211) {
                            window.showModalDialog(u+"2DEdit.html", obj, "dialogWidth=400px;dialogHeight=530px;status=no");
                        } else if (obj.type === 243) {
                            window.showModalDialog(u+"2DEdit.html", obj, "dialogWidth=400px;dialogHeight=570px;status=no");
                        }
                        if (obj.type === 240) {
                            window.showModalDialog(u+"2DEdit.html", obj, "dialogWidth=400px;dialogHeight=560px;status=no");
                        }
                ...
                if (obj.type === 250 || obj.type === 251 || obj.type === 252 || obj.type === 253 || obj.type === 254 || obj.type === 255 || obj.type === 256 || obj.type === 257 || obj.type === 258 || obj.type === 259 || obj.type === 260) { //军标编辑
                    ...
                    if (obj.type === 250 || obj.type === 251 || obj.type === 252 || obj.type === 253 || obj.type === 254 || obj.type === 255 || obj.type === 256 || obj.type === 260) {
                        window.showModalDialog(u+"MilitaryTagData.html", obj, "dialogWidth=400px;dialogHeight=450px;status=no");
                    } else {
                        window.showModalDialog(u+"MilitaryTagData.html", obj, "dialogWidth=400px;dialogHeight=350px;status=no");
                    }
                ...
                if (obj.type === 216 || obj.type === 202 || obj.type === 207 || obj.type === 203 ||
                    ...
                    if (obj.type === 202) { //长方体
                        ...
                        obj.heightValue = currentElement.Height;
                        obj.linecolor = '#' + currentElement.LineColor.toString(16);
                        obj.drawFrameEnable = currentElement.DrawFrameEnable;
                    } else if (obj.type === 203) { //圆柱
                    ...
                    obj.earth = earth;
                    if (obj.type === 206) {
                        window.showModalDialog(u+"3DEdit.html", obj, "dialogWidth=400px;dialogHeight=550px;status=no");
                    } else if (obj.type === 216) {
                        window.showModalDialog(u+"3DEdit.html", obj, "dialogWidth=400px;dialogHeight=420px;status=no");
                    } else if (obj.type === 207) {
                        window.showModalDialog(u+"3DEdit_ex.html", obj, "dialogWidth=400px;dialogHeight=550px;status=no");
                    } else {
                        window.showModalDialog(u+"3DEdit.html", obj, "dialogWidth=400px;dialogHeight=520px;status=no");
                    }
                    ...
                    if (obj.type === 202) { //长方体
                        ...
                        docElement.getElementsByTagName("Height")[0].text = obj.heightValue;
                        if(docElement.getElementsByTagName("DrawFrameEnable").length>0){
                            docElement.getElementsByTagName("DrawFrameEnable")[0].text = obj.drawFrameEnable;
                        }
                        if(docElement.getElementsByTagName("LineColor").length>0){
                            docElement.getElementsByTagName("LineColor")[0].text = obj.linecolor;
                        }
                    } else if (obj.type === 203) { //圆柱
                ...
                if (obj.type === 280 || obj.type === 229 || obj.type === 217 || obj.type === 209 || obj.type === 244) {
                    ...
                    if (obj.type === 229) {
                        ...
                        window.showModalDialog(u+"modelData.html", obj, "dialogWidth=370px;dialogHeight=240px;status=no");
                        if (obj.click === "false") {
                            return;
                        }
                        ...
                        window.showModalDialog(u+"pictureData.html", obj, "dialogWidth=370px;dialogHeight=300px;status=no");
                        if (obj.click === "false") {
                            return;
                        }
                        ...
                        obj.selectable = currentElement.Selectable;
                        obj.editable = currentElement.Editable;
                        window.showModalDialog(u+"iconData.html", obj, "dialogWidth=400px;dialogHeight=540px;status=no");
                        if (obj.click === "false") {
                            return;
                        }
                        ...
                        //obj.desc=elements[i].getElementsByTagName("Description")[0].text;
                        window.showModalDialog(u+"SimpleBuiliding.html", obj, "dialogWidth=370px;dialogHeight=400px;status=no");
                        if (obj.click === "false") {
                            return;
                        }
                        ...
                        window.showModalDialog(u+"gridData.html", obj, "dialogWidth=370px;dialogHeight=260px;status=no");
                        if (obj.click === "false") {
                            return;
                        }
                        docElement.getElementsByTagName("Height")[0].text = obj.height;
    ...
    function setBtnStatus(enabledAry, disabledAry) {
    if (enabledAry) {
        for (var i = enabledAry.length - 1; i >= 0; i--) {
            enabledAry[i].attr("disabled", false);
        };
    }
    if (disabledAry) {
        for (var i = disabledAry.length - 1; i >= 0; i--) {
            disabledAry[i].attr("disabled", true);
        };
    }
};
    /**
    ...
    var getUserdata = function(filename) {
        ...
        if (xmlData == "") {
            // var userdataXml = "<Xml><ElementDocument id='" + earth.Factory.createGuid() + "' name='钻孔数据' im0='folderOpen.gif' type='folder' checked='0' open='false'></ElementDocument><ElementDocument id='" + earth.Factory.createGuid() + "' name='地质模型数据' im0='folderOpen.gif' type='folder' checked='0' open='false'></ElementDocument><ElementDocument id='" + earth.Factory.createGuid() + "' name='剥层模型数据' im0='folderOpen.gif' type='folder' checked='0' open='false'></ElementDocument><ElementDocument id='" + earth.Factory.createGuid() + "' name='剪切模型数据' im0='folderOpen.gif' type='folder' checked='0' open='false'></ElementDocument><ElementDocument id='" + earth.Factory.createGuid() + "' name='隧道模型数据' im0='folderOpen.gif' type='folder' checked='0' open='false'></ElementDocument><ElementDocument id='" + earth.Factory.createGuid() + "' name='填挖方模型数据' im0='folderOpen.gif' type='folder' checked='0' open='false'></ElementDocument><ElementDocument id='" + earth.Factory.createGuid() + "' name='深度利用分析模型数据' im0='folderOpen.gif' type='folder' checked='0' open='false'></ElementDocument></Xml>";
    ...
    var getAllIconObjs = function(currUserdataDoc) {
        ...
        for (var i = 0; i < element.length; i++) {
            ...
            if (objType === 250 || objType === 251 || objType === 252 || objType === 253 ||
                ...
                userdataArr.push(billboard);
                objArr.push(obj);
            } else if (Number(obj.type) == 291) {
                var guid = obj.id;
                var separated = elementObj.getElementsByTagName("Separated")[0].text;
                var tunnelAttached = elementObj.getElementsByTagName("TunnelAttached")[0].text;
                var tunnelHeight = 0;
                var tunnelWidth = 0;
                var tunnelDepth = 0;
                var tunnelPointCount = 0;
                var tunnelPoints = earth.Factory.CreateVector3s();
                if (tunnelAttached == "true") {
                    tunnelHeight = parseInt(elementObj.getElementsByTagName("TunnelHeight")[0].text);
                    tunnelWidth = parseInt(elementObj.getElementsByTagName("TunnelWidth")[0].text);
                    tunnelDepth = parseInt(elementObj.getElementsByTagName("TunnelDepth")[0].text);
                    var elementPoints = elementObj.getElementsByTagName("TunnelPoints")[0];
                    tunnelPointCount = parseInt(elementPoints.getElementsByTagName("Count")[0].text);
                    var elementAllPoints = elementPoints.getElementsByTagName("Point");
                    for (var i = 0; i < tunnelPointCount; ++i) {
                        var x = parseFloat(elementAllPoints[i].getElementsByTagName("X")[0].text);
                        var y = parseFloat(elementAllPoints[i].getElementsByTagName("Y")[0].text);
                        var z = parseFloat(elementAllPoints[i].getElementsByTagName("Z")[0].text);
                        tunnelPoints.Add(x, y, z);
                    }

                }
                var layerCount = elementObj.getElementsByTagName("LayerCount")[0].text;
                var polyhedrons = elementObj.getElementsByTagName("PolyhedronCount");
                var soilLayerModel = earth.Factory.CreateElementSoilLayerModel(guid, "SoilLayerModel");
                soilLayerModel.BeginUpdate();
                if (tunnelAttached == "true") {
                    soilLayerModel.AttachTunnelInfo(tunnelPoints, tunnelDepth, tunnelWidth, tunnelHeight);
                }
                for (var layer = 0; layer < layerCount; ++layer) {
                    var name = elementObj.getElementsByTagName("Name")[layer].text;
                    var color = parseInt(elementObj.getElementsByTagName("Color")[layer].text);
                    soilLayerModel.AddNewLayer(name, color);
                    for (var poly = 0; poly < parseInt(polyhedrons[layer].text); ++poly) {
                        soilLayerModel.AddNewPolyhedronByLoadingFromFile(name, earth.RootPath + "userdata\\SoilLayers\\" + guid + '\\' + layer + "_" + poly);
                    }
                }
                soilLayerModel.name = obj.name;
                soilLayerModel.Visibility = true;
                soilLayerModel.Separated = separated;
                // soilLayerModel.tunnelAttached = tunnelAttached;
                soilLayerModel.SphericalTransform.SetLocationEx(obj.longitude, obj.latitude, obj.altitude);
                soilLayerModel.SphericalTransform.SetRotationEx(rotation[0], rotation[1], rotation[2]);
                soilLayerModel.SphericalTransform.SetScaleEx(scale[0], scale[1], scale[2]);
                soilLayerModel.EndUpdate();
                earth.AttachObject(soilLayerModel);
                setTransform(soilLayerModel, elementObj);
                attachObject(checked, soilLayerModel);
                userdataArr.push(soilLayerModel);
                objArr.push(obj);

            } else if (Number(obj.type) == 292) {
                var guid = obj.id;
                var elementDrill = earth.Geology.LoadDrillDataFromSavedFile(guid, earth.RootPath + "userdata\\DrillData\\" + guid + '.data');
                attachObject(checked, elementDrill);
                userdataArr.push(elementDrill);
                objArr.push(obj);
     ...
     * 右键编辑功能
     */
    $("#divEditUserdata").bind("click", function() {
        var zTree = $.fn.zTree.getZTreeObj("userdataTree");
    ...
    //任意节点(根目录除外)右键新建文件夹
    $("#btnNew").off().on("click", function() {
        addFolder();
    });

    //根目录右键新建文件夹
    $("#btnNewRoot").off().on("click", function() {
        addFolder();
    });

    //树节点右键重命名,只针对文件夹
    $("#renameFolder").off().on("click", function() {
        updateFolder();
    });
    ...
    //树右键删除文件夹
    $("#deleteFolder").off().on("click", function() {
        var zTree = $.fn.zTree.getZTreeObj("userdataTree");
        var nodes = zTree.getSelectedNodes();
        var node = nodes[0];
        //alert(node.id);
     ...
     */
    $("#deleteRootFolder").off().on("click", function() {
        var zTree = $.fn.zTree.getZTreeObj("userdataTree");
        var nodes = zTree.getSelectedNodes();
        var node = nodes[0];
    ...
    //右键删除子节点
    $("#divDeleteUserdata").off().on("click", function() {
    ...
    var ID = 0;
    var treeData = [];
    var initTree = function(userdataTree, showTree) {
        ...
        userTreeData = getElementsList(userDataDoc, "ElementDocument");
     ...
     */
    function getElementsList(xml, rootName) {
        if (xml == null) {
            return;
        }
        ...
        for (var t = 0; t < length; t++) {
            var id = list[t].getAttribute("id");
            var checked = list[t].getAttribute("checked");
            var open = list[t].getAttribute("open");
            var name = list[t].getAttribute("name");
    ...
    function recursion(x, originID) {
        for (var i = 0, max = x.length; i < max; i++) {
            ...
            if (name == "ElementFolder" && element.childNodes) {
                ...
                recursion(element.childNodes, nID);
            }
        }
    }
    ...
    function updateButtonStatus() {
        ...
        if (node) {
            if (node.level == 0) { //根节点
                setBtnStatus([$("#mImportUndergroundData")], [$("#mCreateSoilLayer"), $("#mSeparateSoilLayers"), $("#mDig"), $("#mDigTunnel"), $("#mTrackTunnel"), $("#mDepthLayoutAnalysis"), $("#mSoilExcavationAndFill"), $("#mSoilLayerLegend")]);
            } else if (node.level == 1) { //模型节点
                if (node.checked) {
                    var editObj = _getElementByID(node.id);
                    if (editObj.Rtti == 292) { // 钻孔
                        setBtnStatus([$("#mImportUndergroundData"), $("#mCreateSoilLayer")], [$("#mSeparateSoilLayers"), $("#mDig"), $("#mDigTunnel"), $("#mSoilExcavationAndFill"), $("#mDepthLayoutAnalysis"), $("#mSoilLayerLegend"), $("#mTrackTunnel")]);
                    } else if (editObj.Rtti == 291) { // 模型
                        if (editObj.TunnelAttached) {
                            setBtnStatus([$("#mImportUndergroundData"), $("#mTrackTunnel"), $("#mSoilLayerLegend")], [$("#mCreateSoilLayer"), $("#mSeparateSoilLayers"), $("#mDig"), $("#mDigTunnel"), $("#mDepthLayoutAnalysis"), $("#mSoilExcavationAndFill")]);
                        } else if (editObj.Separated) {
                            setBtnStatus([$("#mImportUndergroundData"), $("#mSoilLayerLegend")], [$("#mCreateSoilLayer"), $("#mSeparateSoilLayers"), $("#mDig"), $("#mDigTunnel"), $("#mDepthLayoutAnalysis"), $("#mTrackTunnel"), $("#mSoilExcavationAndFill")]);
                        } else {
                            setBtnStatus([$("#mImportUndergroundData"), $("#mSeparateSoilLayers"), $("#mDig"), $("#mDigTunnel"), $("#mSoilExcavationAndFill"), $("#mDepthLayoutAnalysis"), $("#mSoilLayerLegend")], [$("#mTrackTunnel"), $("#mCreateSoilLayer"), ]);
                        }
                    } else {
                        setBtnStatus([$("#mImportUndergroundData")], [$("#mCreateSoilLayer"), $("#mSeparateSoilLayers"), $("#mDig"), $("#mDigTunnel"), $("#mSoilExcavationAndFill"), $("#mDepthLayoutAnalysis"), $("#mSoilLayerLegend"), $("#mTrackTunnel")]);
                    }
    ...
    function ChangeUsablilitiesOfSoilLayerRelatedButtons(model) {}

    //树节点单击
    var zTreeOnClick = function(event, treeId, node) {
        if (node && node.isParent) {} else {
            var editObj = _getElementByID(node.id);
            if (editObj.Rtti == 291) {
                // 设置当前模型
                earth.Geology.ActiveSoilLayerModel = editObj;
            } else if (editObj.Rtti == 292) {
                // 设置当前钻孔
                earth.Geology.ActiveDrillData = editObj;
            }
        }
        updateButtonStatus();
    };
    ...
    var getuserdataArr = function() {
        return userdataArr;
    };

    var gettreePIDObject = function() {
        return treePIDObject;
    }
    ...
    userdata.createElement = createElement;
    userdata.getuserdataArr = getuserdataArr;
    userdata.gettreePIDObject = gettreePIDObject;
    return userdata;
