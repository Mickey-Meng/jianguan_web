(function(ctx, $, opt){
	...
	var _getLayerByName = function(name, layer, exact) {
	    var _getLayer = function(layer, name, exact) {
	        if ((exact === true && layer.Name == name) ||
	            (!exact && layer.Name.toLowerCase().indexOf(name.toLowerCase()) >= 0)) {
	            return layer;
	        } else {
	            for (var i = 0; i < layer.GetChildCount(); i++) {
	                var lyr = _getLayer(layer.GetChildAt(i), name, exact);
	                if (lyr) {
	                    return lyr;
	                }
	            }
	...
	var getSELayerByName = function(name, layer, exact, parentName) {
	    if(!parentName){
	        return _getLayerByName(name, layer, exact);
	    }
	    var pl = _getLayerByName(parentName, layer, exact);
	    if(pl){
	        return _getLayerByName(name, pl, exact);
	    }
	    return null;
	};
	...
	var xy2blh = function(datum, point){
		if(datum){
			return datum.src_xy_to_des_BLH(point.x, point.y, point.z || 0);
		}
		return point;
	};
	var blh2xy = function(datum, point){
		if(datum){
			return datum.des_BLH_to_src_xy(point.x, point.y, point.z || 0);
		}
		return point;
	};

	var localSearch = function(layer, keyword, spatialFilter, pageSize) {
	    if(!layer){
	        return null;
	    }

	    var sp = layer.LocalSearchParameter;
	    if (sp == null) {
	        return null;
	    }
	    sp.ClearSpatialFilter();
	    if (spatialFilter != null) {
	        sp.SetFilter("", "");
	        sp.SetSpatialFilter(spatialFilter);
	    }
	    if (keyword) {
	        sp.SetFilter(keyword, "");
	        // sp.PreciseKeyValue = keyword;
	    }

	    sp.PageRecordCount = pageSize || 65535;
	    sp.HasDetail = true;
	    sp.HasMesh = true;
	    var tt = sp.ReturnDataType;
	    sp.ReturnDataType = 1;
	    var result = layer.SearchFromLocal();
	    result.GotoPage(0);
	    sp.ReturnDataType = tt;
	    return result;
	};
	var localSearchByKey = function(earth, layer, key, exect){
		var rect = layer.LonLatRect;
        var v3s = null;
        if(exect){
        	v3s = earth.Factory.CreateVector3s();
        	v3s.Add(rect.West, rect.North, 0);
	        v3s.Add(rect.East, rect.North, 0);
	        v3s.Add(rect.East, rect.South, 0);
	        v3s.Add(rect.West, rect.South, 0);
	        v3s = null;
        }
		return localSearch(layer, key, v3s, 10);
	};
	...
	var getAttrHtml2 = function(fields, attr){
		var h = '<div style="overflow:auto;width:205px;height:245px;margin:0px 0px 0px 0px;text-align:center;">' +
    ...
    var anaPipeConnection = function(options){
    	var u = sfmMm.getServiceUrl('stAnalysis') + 'anaPipeConnection';
    	var p = {
			type: 'GET',
			dataType: 'json',
			url: u,
	...
	PageHelper.prototype /*= earthCtx.PageHelper */= {
	...
	HighlightManager.prototype = {
		...
		showHighlight: function(model, locate){
			if(model){
				model.showHighLight();
				this.data.hlObj.push(model);
				if(locate){
					var lon = model.SphericalTransform.Longitude;
					var lat = model.SphericalTransform.Latitude;
					var alt = model.SphericalTransform.Altitude;
					this.data.earth.GlobeObserver.GotoLookat(lon, lat, alt, 50, 45, 0, 20);
				}
			}
			return this;
		},
	...
	ctx.SFM.queryHelper = {
		...
		getSELayerByName: getSELayerByName,
		identify: identify,
		getDaily: getDaily,
		...
		localSearchByKey: localSearchByKey,
		...
		showHtmlBalloon: earthCtx && earthCtx.showHtmlBalloon,
		clearHtmlBalloons: earthCtx && earthCtx.clearHtmlBalloons,

		PageHelper: PageHelper,
		HighlightManager: HighlightManager
	};
	if(earthCtx){
		earthCtx.queryHelper = ctx.SFM.queryHelper;
	}
