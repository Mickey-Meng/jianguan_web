...
var QueryObject = {
    ...
    parseResult : function (result,layer,standardName) {
        ...
        if (json.Result.num <= 0) {
            ...
            //只有一个记录
            if(type=="point"){
                var us_key_point = ectx.getName("US_KEY",0,true);
                var pointType="管点";
                ...
                $("#importResult>tbody").append(template.replace("$INDEX", records[us_key_point])
                    ...
                    .replace("$US_KEY",records[ectx.getName("US_KEY",0,true)])//编号
                    .replace("$LAYER",layer.Name));
            }else{
                var us_key_line = ectx.getName("US_KEY",1,true);
                $("#importResult>tbody").append(template.replace("$INDEX", records[us_key_line])
                    ...
                    .replace("$US_SIZE", records[ectx.getName("US_SIZE",1,true)])//管径
                    .replace("$US_PMATER", records[ectx.getName("US_PMATER",1,true)])//材质
                    .replace("$US_LTTYPE", records[ectx.getName("US_LTTYPE",1,true)])//埋设方式
                    .replace("$US_STATUS", records[ectx.getName("US_STATUS",1,true)])//废弃==状态
                    .replace("$LAYER",layer.Name));
            }
        } else {
            //多条记录
            for (var i = 0; i < records.length; i++) {
                if(type=="point"){
                    ...
                    $("#importResult>tbody").append(template.replace("$INDEX", records[i][us_key_point])
                        ...
                        .replace("$US_KEY",records[i][ectx.getName("US_KEY",0,true)])//编号
                        //.replace("$US_PMATER",records[i][ectx.getName("US_PMATER",0,true)])
                        .replace("$LAYER",layer.Name));
                }else{
                    var us_key_line = ectx.getName("US_KEY",1,true);
                    $("#importResult>tbody").append(template.replace("$INDEX", records[i][us_key_line])
                        ...
                        .replace("$US_KEY", us_key_line)//编号
                        .replace("$US_SIZE", records[i][ectx.getName("US_SIZE",1,true)])//管径
                        .replace("$US_PMATER", records[i][ectx.getName("US_PMATER",1,true)])//材质
                        .replace("$US_LTTYPE", records[i][ectx.getName("US_LTTYPE",1,true)])//埋设方式
                        .replace("$US_STATUS", records[i][ectx.getName("US_STATUS",1,true)])//废弃==状态
                        .replace("$LAYER",layer.Name));
                }
            }
        }
