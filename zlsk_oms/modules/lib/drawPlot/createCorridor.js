//绘制走廊
var CreateCorridor = function (Cesium,viewer, style) {
...
CreateCorridor.prototype = {
    start: function (callBack) {
        ...
        this.handler.setInputAction(function (evt) { //移动时绘制线
            ...
            var cartesian = that.getCatesian3FromPX(evt.endPosition, that.viewer, [that.corridor]);
            if (that.floatPoint) that.floatPoint.position.setValue(cartesian);
            if (that.positions.length == 2) {
                if (!that.Cesium.defined(that.corridor)) {
                    that.corridor = that.createCorridor();
                }
            }
            if (that.corridor) {
                that.positions.pop();
        ...
        this.handler.setInputAction(function (evt) { //单机开始绘制
            ...
            var cartesian = that.getCatesian3FromPX(evt.position, that.viewer, [that.corridor]);
            that.state = 1;
			...
			// 	this.prompt = null;
			// }
            if (callBack) callBack(that.corridor);
    ...
    startModify: function (callback) {
        ...
        this.modifyHandler.setInputAction(function (evt) {
            ...
            var cartesian = that.getCatesian3FromPX(evt.endPosition, that.viewer, [that.corridor, that.modifyPoint]);
            if (cartesian) {
        ...
        this.modifyHandler.setInputAction(function (evt) {
            ...
            var cartesian = that.getCatesian3FromPX(evt.position, that.viewer, [that.corridor, that.modifyPoint]);
            that.modifyPoint.position.setValue(cartesian);
            that.positions[that.modifyPoint.wz] = cartesian;
