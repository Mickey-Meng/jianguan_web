var CreateImgRectangle = function(Cesium,viewer, style) {
...
CreateImgRectangle.prototype = {
	start: function(callBack) {
		var that = this;
		this.handler.setInputAction(function(evt) { //单机开始绘制
			var cartesian = that.getCatesian3FromPX(evt.position, that.viewer,[that.imgRectangle]);
			if (that.points.length == 0) {
				...
				that.imgRectangle = that.createImgRectangle();
			}
	...
	createByPositions: function(lnglatArr, callBack) { //通过传入坐标数组创建面
		...
		if (!position) return;
		this.imgRectangle = this.createImgRectangle(position);
		callBack(this.imgRectangle);
		this.state = 1;
	},
	openModify: false,
	startModify: function() {
		...
		this.modifyHandler.setInputAction(function(evt) { //移动时绘制面
			...
			var cartesian = that.getCatesian3FromPX(evt.endPosition, that.viewer, [that.imgRectangle, that.modifyPoint]);
			if (cartesian) {
		...
		this.modifyHandler.setInputAction(function(evt) {
			...
			var cartesian = that.getCatesian3FromPX(evt.position, that.viewer, [that.imgRectangle, that.modifyPoint]);
			that.modifyPoint.position.setValue(cartesian);
			that.positions[that.modifyPoint.wz] = cartesian;
	...
	createImgRectangle: function() {
		if (!this.coordinates) return;
		var that = this;
		var imgRectangle = this.viewer.entities.add({
			rectangle : {
				// coordinates : coordinates,
				coordinates : new that.Cesium.CallbackProperty(function() {
					return that.coordinates
				}, false),
				material : that.style.material || new that.Cesium.ImageMaterialProperty({
					image:"modules/widget/zhab/images/plot/gugong.jpg",
				}),
