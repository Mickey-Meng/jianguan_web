var CreateCircle = function(Cesium,viewer, style) {
...
CreateCircle.prototype = {
	start: function(callBack) {
		...
		this.handler.setInputAction(function(evt) { //移动时绘制面
			...
			if (that.positions.length == 2) {
				if (!that.Cesium.defined(that.circle)) {
					that.circle = that.createCircle(that.style);
					that.circle.isFilter = true;
					that.circle.objId = that.objId;
	...
	screenToGeo :function(position){
		var screen = new this.Cesium.Cartesian2(position.x,position.y);
		var world = this.viewer.scene.globe.pick(this.viewer.camera.getPickRay(screen),this.viewer.scene);
		var Geography = this.viewer.scene.globe.ellipsoid.cartesianToCartographic(world);
		var geo = {x:this.Cesium.Math.toDegrees(Geography.longitude),y:this.Cesium.Math.toDegrees(Geography.latitude)}
		return geo;
	  }
  ,
		...
		this.modifyHandler.setInputAction(function(evt) { //移动时绘制面
			...
			if (cartesian) {
				...
				if (that.movePointType =="yd") {
					...
					var  aaa = that.screenToGeo(cartesian2);
					var  bbb = that.screenToGeo(p1Cartesian2);
					var p = that.scalePointToLen(aaa,bbb,that.radius);
					var newC2 = null
	...
	createCircle: function() {
		...
		return this.viewer.entities.add({
			...
			ellipse : {
				...
				material : that.style.material || that.Cesium.Color.GREEN.withAlpha(0.5),
