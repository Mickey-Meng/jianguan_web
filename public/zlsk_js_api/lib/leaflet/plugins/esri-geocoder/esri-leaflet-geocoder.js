	  ...
	  _processFindResponse: function (response) {
	    ...
	    for (var i = 0; i < response.locations.length; i++) {
	      ...
	      results.push({
	        ...
	        latlng: L.latLng(location.feature.geometry.y, location.feature.geometry.x),//crs.unproject(location.feature.geometry),
	  ...
	  _processFindAddressCandidatesResponse: function (response) {
	    ...
	    for (var i = 0; i < response.candidates.length; i++) {
	      ...
	      results.push({
	        ...
	        latlng: L.latLng(candidate.location.y, candidate.location.x),  //crs.unproject(candidate.location),
	...
	var ReverseGeocode = esriLeaflet.Task.extend({
	  ...
	  run: function (callback, context) {
	    return this.request(function (error, response) {
	      ...
	      if (!error) {
	        result = {
	            latlng: L.latLng(response.location.y, response.location.x),//crs.unproject(response.location),
	...
	var FeatureLayerProvider = esriLeaflet.FeatureLayerService.extend({
	  ...
	  _featureBounds: function (feature) {
	      var geojson = L.geoJson(feature, {
	          ...
	          //    return crs.unproject({ x: coordinates[0], y: coordinates[1] });
	          //}
	      });
