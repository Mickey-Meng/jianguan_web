...
【3】按住鼠标中键并移动，可进行地图旋转和俯仰，按住向上正视，按住向下俯视，按住向左顺时针旋转，按住向右逆时针旋转`)
    ...
    checkNode(node, select) {
      if (node.label && node.label == "诸暨影像") {
        window.viewer.scene.imageryLayers.get(0).show = select;
      } else if (node.label && node.label == "诸暨地形") {
        select ? viewer.terrainProvider = viewer.terrainProviderBak : viewer.terrainProvider = new Cesium.EllipsoidTerrainProvider({})
      } else if (node.type && node.type == "C3DTILES") {
        viewer.getLayerByName(node.label).show = select;
      }
