...
<script>
  ...
  import {getPerMonthSafeData,getDayData} from '@/api/safe'
  export default {
    name: '',
    data() {
      return {
    ...
    methods: {
      ...
      initData(time){
        getPerMonthSafeData({askTime:time}).then(res => {
          let num=null
          const findObj = (item,result) => {
            for (let i=1;i<=this.dayLength;i++){
              if(i<10){
                num = '0'+i
              }else{
          ...
          if (data){
            data.forEach(item => {
              if(item.dayDatas && item.dayDatas.length >0){
                item.dayDatas.forEach(result => {
                  let yearMonth = result.uploadTime.split(' ')[0]
                  let day =yearMonth.split('-')[2]
                  result.date = day
                  findObj(item,result)
                })
              }
