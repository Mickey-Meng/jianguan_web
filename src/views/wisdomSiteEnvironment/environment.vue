...
import monitoring from "@/views/wisdomSiteEnvironment/indexComponent/monitoring";
import topVideo from "@/views/wisdomSiteEnvironment/indexComponent/topVideo";
import { getVideoToken } from "@/api/wisdomSite";
import {getMonitoring} from "@/api/project";
let viewer, em, player, popupId, videoMarker, popup;
export default {
  name: "",
  ...
  created() {
    ...
    videoMarker = new Cesium.CustomDataSource('videoMarker');
      // videoMarker.show = false;
    viewer.dataSources.add(videoMarker);
  ...
  methods: {
    ...
    flySite(row) {
      ...
      if (row && row.geom) {
        const geom = row.geom.split(",");
        row.position = Cesium.Cartesian3.fromDegrees(geom[0], geom[1])

        viewer.camera.flyTo({
          destination : Cesium.Cartesian3.fromDegrees(parseFloat(geom[0]), parseFloat(geom[1]), 100.0)
        });
      }
    },
    initData() {
    ...
    initMarker(data) {
      ...
      data.forEach((item) => {
        ...
        if (geom) {
          geom = geom.split(",")
          videoMarker.entities.add({
            edata: item,
            position : Cesium.Cartesian3.fromDegrees(geom[0], geom[1]),
            label: {
              text: name,
              showBackground: true,
              distanceDisplayCondition: new Cesium.DistanceDisplayCondition(0, 5000000),
              pixelOffset: new Cesium.Cartesian2(70, -75),
      ...
      handler.setInputAction(function (movement) {
        ...
        if (feature) {
            let { edata, position } = feature.id;
            // // player && player.stop();
            // popupId && zeh.closePopup(popupId);
            if (edata.type == 2) {
              that.addPopup(edata, position)
            }
        } else {
          popup.show = false;
        }
      ...
      viewer.clock.onTick.addEventListener(function () {
        if (popup) {
          if (popup.show) {
            popup.ele.style.display = ''
            const screenPosition = computeScreenSpacePosition(viewer.scene, popup.position instanceof Cesium.Cartesian3 ? popup.position : popup.position._value, new Cesium.Cartesian2());
            if (screenPosition) {
              const _height = Math.floor(popup.ele.clientHeight);
              const _screenPositionY = Math.floor(screenPosition.y + 0.25) - _height;

              popup.ele.style.position = "absolute"

              popup.ele.style.left = `${Math.floor(screenPosition.x - 350 / 2 + 0.25)
                }px`;
              popup.ele.style.top = `${_screenPositionY - 70}px`;
            }
          } else {
            popup.ele.style.display = 'none'
          }
        }
      });
    },
    addPopup(edata, position) {
      ...
      if (!popup) {
        ...
        popup.ele = ele;
        popup.type = edata.type;

        let html = `<div id="mon_video"></div>`;
        ele.innerHTML = html;
      } else {
        if (player) {
          player.stop();
          player = undefined;
          popup.ele.removeChild(document.getElementById("mon_video"));

          let html = `<div id="mon_video"></div>`;
          popup.ele.innerHTML = html;
        }
      }

      popup.position = position || edata.position;
      popup.show = true;
  ...
  destroyed() {
    videoMarker && viewer.dataSources.remove(videoMarker);
    if (player) {
      player.stop();
      player = undefined;
      popup.ele.removeChild(document.getElementById("mon_video"));
