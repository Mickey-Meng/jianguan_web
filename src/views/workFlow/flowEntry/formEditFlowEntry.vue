          ...
          <el-col :span="8">
            <el-row type="flex" justify="end" align="middle" style="height: 60px;">
              <el-button size="small" @click="onPrevClick"
                :disabled="activeStep === SysFlowEntryStep.BASIC.id"
              >
                上一步
              </el-button>
              <el-button size="small" @click="onNextClick" :disabled="activeStep === SysFlowEntryStep.PROCESS_DESIGN.id">下一步</el-button>
              <el-button size="small" type="primary" @click="onClose(false)">退出</el-button>
            </el-row>
          </el-col>
        </el-row>
      </el-header>
      <el-main style="background: #EBEEF5; padding: 10px;height:calc(100% - 60px)">
        <el-row type="flex" justify="center" style="height: 100%;">
          <!-- 流程基础信息设置 -->
          <el-col v-if="activeStep === SysFlowEntryStep.BASIC.id" class="main-box" style="width: 600px;" :span="9">
            <el-form ref="entryBasicInfo" class="full-width-input" size="small" :model="formFlowEntryData" :rules="formRules"
              ...
              <el-col :span="24">
                <el-form-item label="表单类型" prop="bindFormType">
                  <el-select v-model="formFlowEntryData.bindFormType" placeholder="" @change="onBindFormTypeChange" :disabled="isEdit">
                    <el-option :label="SysFlowEntryBindFormType.ROUTER_FORM.name" :value="SysFlowEntryBindFormType.ROUTER_FORM.id" />
                    <!-- <el-option :label="SysFlowEntryBindFormType.ONLINE_FORM.name" :value="SysFlowEntryBindFormType.ONLINE_FORM.id" /> -->
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="24">
              ...
              <el-col :span="24" v-if="formFlowEntryData.bindFormType === SysFlowEntryBindFormType.ROUTER_FORM.id">
          ...
          <!-- 流程变量设置 -->
          <el-col v-if="activeStep === SysFlowEntryStep.PROCESS_VARIABLE.id" class="main-box" :span="16">
            <el-table :data="processVariableList" header-cell-class-name="table-header-gray" key="processVariableList">
              ...
              <el-table-column label="变量类型">
                <template slot-scope="scope">
                  <el-tag v-if="scope.row.variableType === SysFlowVariableType.TASK.id"
                    size="mini" effect="dark"
                    type="primary">
                    {{SysFlowVariableType.TASK.name}}
                  </el-tag>
                  <el-tag v-if="scope.row.variableType === SysFlowVariableType.INSTANCE.id"
                    size="mini" effect="dark"
                    type="success">
                    {{SysFlowVariableType.INSTANCE.name}}
                  </el-tag>
                </template>
              </el-table-column>
          ...
          <!-- 流程设计 -->
          <el-col v-if="activeStep === SysFlowEntryStep.PROCESS_DESIGN.id" class="main-box" :span="24" style="min-width: 1100px; padding: 0px">
...
export default {
  ...
  data () {
    return {
      ...
      formFlowEntryData: {
        ...
        categoryId: '1525342004219744256',
        bindFormType: 1,
        pageId: undefined,
  ...
  methods: {
    ...
    loadCategoryIdDropdownList () {
      api.getFlowCategoryListDict({}).then(res => {
        this.categoryIdWidget.dropdownList = res.data;
      });
    },
    // 获取在线表单列表
    ...
    onPrevClick () {
      switch (this.activeStep) {
        case this.SysFlowEntryStep.PROCESS_VARIABLE.id:
          this.activeStep = this.SysFlowEntryStep.BASIC.id;
          break;
        case this.SysFlowEntryStep.PROCESS_DESIGN.id:
          this.activeStep = this.SysFlowEntryStep.PROCESS_VARIABLE.id;
          break;
      }
    },
    onNextClick () {
      switch (this.activeStep) {
        case this.SysFlowEntryStep.BASIC.id:
          ...
          break;
        case this.SysFlowEntryStep.PROCESS_VARIABLE.id:
          this.activeStep = this.SysFlowEntryStep.PROCESS_DESIGN.id;
          break;
      }
    },
    // 初始化流程基础信息
    initFlowEntryInfo () {
      this.formFlowEntryData = {
        ...
        bindFormType: this.SysFlowEntryBindFormType.ONLINE_FORM.id,
        pageId: undefined,
        defaultFormId: undefined,
        defaultRouterName: undefined,
        bpmnXml: undefined
      ...
      FlowEntryController.view({
        ...
        if (this.formFlowEntryData.bindFormType === this.SysFlowEntryBindFormType.ONLINE_FORM.id) {
  ...
  computed: {
    isEdit () {
      console.log(this.flowEntry, this.formFlowEntryData.entryId)
  ...
  mounted () {
    ...
    this.loadCategoryIdDropdownList();
    if (this.isEdit) {
