    ...
    >
      <filter-box :item-width="350">
        ...
        <el-form-item label="流程标识" prop="formFilter.processDefinitionKey">
          <el-input
            class="filter-item"
            v-model="formFlowEntry.formFilter.processDefinitionKey"
            :clearable="true"
            placeholder="流程标识"
          />
        </el-form-item>
        ...
        <el-button
          ...
          :plain="true"
          size="mini"
          @click="refreshFormFlowEntry(true)"
          >查询</el-button
        >
        <el-button
          slot="operator"
          ...
          <el-table-column label="发布状态" prop="status">
            <template slot-scope="scope">
              <el-tag
                size="mini"
                :type="
                  scope.row.status === SysFlowEntryPublishedStatus.PUBLISHED
                    ? 'success'
                    : 'warning'
                "
              >
          ...
          <el-table-column label="操作" fixed="right" width="250px">
            <template slot-scope="scope">
              <el-button
                ...
                size="mini"
                :disabled="
                  !checkPermCodeExist('formFlowEntry:formFlowEntry:start') ||
                  !(
                    scope.row.mainFlowEntryPublish &&
                    scope.row.mainFlowEntryPublish.activeStatus
                  )
                "
              >
              ...
              <el-button
                ...
                size="mini"
                :disabled="
                  !checkPermCodeExist('formFlowEntry:formFlowEntry:update')
                "
              >
                编辑
              </el-button>
...
export default {
  ...
  data() {
    return {
      ...
      formFlowEntry: {
        ...
        // flowEntryWidget: new TableWidget(
        //   this.loadFlowEntryWidgetData,
        //   this.loadFlowEntryVerify,
        //   true,
        //   false,
        //   'entryId',
        //   1
  ...
  methods: {
      ...
      params = {
        ...
        flowEntryDtoFilter: {
          ...
          processDefinitionName:
            this.formFlowEntry.formFilterCopy.processDefinitionName,
          processDefinitionKey:
            this.formFlowEntry.formFilterCopy.processDefinitionKey,
          status: this.formFlowEntry.formFilterCopy.status,
        },
      };
      return new Promise((resolve, reject) => {
        FlowEntryController.list(this, params)
          .then((res) => {
            resolve({
     ...
     */
    loadFlowEntryVerify() {
      this.formFlowEntry.formFilterCopy.categoryId =
        this.formFlowEntry.formFilter.categoryId;
      this.formFlowEntry.formFilterCopy.processDefinitionName =
        this.formFlowEntry.formFilter.processDefinitionName;
      this.formFlowEntry.formFilterCopy.processDefinitionKey =
        this.formFlowEntry.formFilter.processDefinitionKey;
      this.formFlowEntry.formFilterCopy.status =
        this.formFlowEntry.formFilter.status;
      return true;
    },
    /**
     * 流程分类下拉数据获取函数
    ...
    loadCategoryIdDropdownList() {
      return new Promise((resolve, reject) => {
        let params = {};
        FlowDictionaryController.dictFlowCategory(this, params)
          .then((res) => {
            resolve(res.getList());
          })
          .catch((e) => {
    ...
    onPublishedClick(row) {
      this.$confirm('是否发布当前工作流设计？')
        .then((res) => {
          ...
          FlowEntryController.publish(this, params)
            .then((res) => {
    ...
    onDeleteFlowEntryClick(row) {
      ...
      this.$confirm('是否删除此流程？')
        .then((res) => {
          FlowEntryController.delete(this, params)
            .then((res) => {
