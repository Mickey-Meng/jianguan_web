    ...
    initMap() {
      ...
      obj.layers = [
        ...
        },
        {
          name: "诸暨白模",
          type: "C3DTILES",
          url: "http://150.158.139.18:26666/data_zlsk/zhujishi/jianzhu/tileset.json",
          clampToGround: true
        }
      ];
      ...
      const URL = `http://36.139.53.154:26666/data_zlsk/zhujishi/jianzhu/tileset.json`;
      const tilesetModel = new Cesium.Cesium3DTileset({
        url: URL
      });
      viewer.scene.primitives.add(tilesetModel).readyPromise.then(tileset => {
        tileset._root.transform = Cesium.Matrix4.fromArray([-0.8636924273282998, -0.5040192367120025, 0, 0, 0.24989311155570004, -0.42821934634905656, 0.8684363098174771, 0, -0.43770860600719297, 0.7500618644062882, 0.4958007420230458, 0, -2794066.456104733, 4787940.347936744, 3143704.4582303525, 1])
          ...
          popupManagement[key].forEach(item => {
            if (item.show) {
              ...
              if (screenPosition) {
                ...
                item.ele.style.left = `${Math.floor(screenPosition.x + 0.25)
                  }px`;
                item.ele.style.top = `${_screenPositionY
                  }px`;
              }
            } else {
