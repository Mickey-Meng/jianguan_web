        ...
        <el-table :data="tableData" style="width: 100%" border height="calc(100% - 48px)" class="have_scrolling">
          ...
          <el-table-column fixed="right" width="120" align="center" label="操作">
            <template slot-scope="{ row, $index }">
              <el-button v-if="editStatus(row)" type="text" size="mini" @click="modify(row)">编辑</el-button>
...
import edit from '../constructionPlanReport/edit.vue';
import detail from '../constructionPlanReport/detail.vue';
...
export default {
  ...
  methods: {
    editStatus(row) {
      ...
      let eq = row.responsiblePersonId == userId.slice(1,-1)
      if (row.reportStatus == -1 && eq) {
        return true;
      }
      if (row.reportStatus == 2 && eq) {
        return true;
      }
      ...
      if(row.responsiblePersonId !== userId.slice(1,-1)) {
        return false;
      }
      return false;
    },
