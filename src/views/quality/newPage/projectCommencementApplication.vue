		...
		<el-header>
			<div class="input-box">
				<div class="input-value">
					<el-date-picker v-model="queryData.endDateStart" type="date" placeholder="计划完工开始日期">
					</el-date-picker>
				</div>
			</div>
			<div class="input-box">
				<div class="input-value">
					<el-date-picker v-model="queryData.endDateEnd" type="date" placeholder="计划完工结束日期">
					</el-date-picker>
				</div>
			</div>
			<div class="input-box">
				<div class="input-value">
					<el-date-picker v-model="queryData.openDateEnd" type="date" placeholder="计划开工结束日期">
					</el-date-picker>
				</div>
			</div>
			<div class="input-box">
				<div class="input-value">
					<el-date-picker v-model="queryData.openDateStart" type="date" placeholder="开工开始日期">
					</el-date-picker>
				</div>
			</div>
		...
		<el-main>
			<div class="container">
				<el-table :data="tableData" style="width: 100%" border height="calc(100% - 48px)"
					...
					<el-table-column prop="buildSectionNames" align="center" label="标段" show-overflow-tooltip>
					</el-table-column>
					<el-table-column prop="contractEndDate" align="center" label="合同规定工期止" show-overflow-tooltip>
					</el-table-column>
					<el-table-column prop="contractOpenDate" align="center" label="合同规定工期起" show-overflow-tooltip>
					</el-table-column>
					<el-table-column prop="endDate" align="center" label="计划完工日期" show-overflow-tooltip>
					</el-table-column>
					<el-table-column prop="openDate" align="center" label="开工日期" show-overflow-tooltip>
					</el-table-column>
...
<script>
	...
	import edit from './projectCommencementApplication/edit';
	import detail from './projectCommencementApplication/detail';
	...
	export default {
		...
		methods: {
			query() {
				...
				api.getProjectOpenList(this.queryData).then((res) => {
			...
			deleteRow(row) {
				this.$confirm('确认是否删除?', '提示', {
					...
					api.deleteProjectOpen(row['id']).then((res) => {
			...
			exportData() {
				...
				api.exportProjectOpenList(this.queryData).then((res) => {
					const reader = new FileReader();
					reader.readAsDataURL(res);
					reader.onload = (e) => {
						const a = document.createElement('a');
						a.download = `项目开工申请清单.xls`;
