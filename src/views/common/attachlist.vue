<template>
	<div>
		<div class="block-line" v-if="editAble">
			<el-upload class="upload-demo" action="" :limit="1" :show-file-list="false" ref="upload" :accept="accept"
		...
		<el-dialog title="预览" :visible.sync="viewFileVisible" :fullscreen="true" width="100%" height="100%">
			...
			<iframe width="100%" height="100%" style="min-height:800px" v-if="viewFileType == 'pdf'" :src="viewFileUrl"
...
export default {
	...
	methods: {
		...
		reviewAttach(row, index) {
			...
			this.viewFileUrl = "/prod-api" + this.$store.getters.lookUrl + row.fileId;
