...
<template>
	<el-container class="container-box">
		<el-header>
			...
			<el-button v-if="$store.getters.rolePerms[0] == 'gly'" type="primary" @click="deleteProcess">删除待办</el-button>
			<el-button v-if="$store.getters.rolePerms[0] == 'gly'" type="primary"
				@click="deleteCopyProcess">删除抄送</el-button>
		</el-header>
		<el-main>
			<div class="container">
...
export default {
	...
	methods: {
		...
		deleteCopyProcess() {
			api.handDeleteCopyProcessInstance({
				processInstanceId: this.processInstanceIdList,
				stopReason: '手动删除'
			}).then(res => {
				this.handlerClose();
